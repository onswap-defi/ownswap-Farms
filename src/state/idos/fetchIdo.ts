...
const fetchIdo = async () => {
  const data = await Promise.all(
    farmsConfig.map(async (farmConfig) => {
      ...
      obj.nextClaim = new BigNumber(nextClaim).toNumber() * 1000
      obj.target = new BigNumber(targetToRaise).toNumber() / 10 ** 18
      obj.projectOwner = projectOwner[0]
      obj.owner = owner[0]
      obj.totalFundLocked = new BigNumber(totalFundLocked).toNumber() / 10 ** 18
      obj.totalFundClaimed = new BigNumber(totalFundClaimed).toNumber() / 10 ** 18
      obj.totalLpLocked = new BigNumber(totalLpLocked).toNumber() / 10 ** 18
      obj.totalLpClaimed = new BigNumber(totalLpClaimed).toNumber() / 10 ** 18
